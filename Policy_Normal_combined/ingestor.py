...
async def check_authorisation(alert):
    async with AsyncOpaClient(host='localhost', port=8181) as client:
        ...
        # Evaluate policy
        input_data = alert
        policy_name = 'ingestor/ingestor_policies' # the endpoint = the ID assigned to the policy stored on the OPA service
...
with open('./ingestor_normal_case_combined_time', "w") as f:
...
for repetition in range(1, num_executions + 1): # from 1 to 10
    ...
    with open('./ingestor_normal_case_combined_time', "a") as f:
